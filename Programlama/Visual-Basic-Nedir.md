...
Adi , soyadi , yasi , Maas98 , Dogum\_Yeri , SANAT\_DALI
...
Burada görmüs oldugunuz gibi 5 adet degisken tanimlanmaktadir.Ad degiskeni string tipinde bir degiskendir.Maas degiskeni Currency tipinde bir degiskendir.D\_tarihi degiskeni Date tipinde bir degiskendir.Adres degiskeni string tipinde bir degiskendir.Sira degiskeni integer tipinde bir degiskendir.
...
B\_sayi = Oran\*A\_sayi

C\_sayi = B\_sayi + A\_sayi - 1000
...
Dim Dogum\_tarihi As Date , Evlenme\_Tarihi As Date

Dogum\_tarihi =#Dec,9,1977#

Evlenme\_Tarihi=#May,15,1998#
...
dizi\_adi(indis\_sirasi)=atanacak\_bilgi
...
Dim dizi\_adi(sayi) As Veri\_Tipi
...
ReDim Preserve s(boyut) As Veri\_tipi
...
DoEditThing Copy|Cut|Paste|Undo\*, On\_Which\_Object\*\*
...
Dim Gwdvar As String, Gwdvar\_Length As Integer

Gwdvar = Space(255)

Gwdvar\_Length = GetSystemDirectory(Gwdvar, 255)

VBSysDir = Left(Gwdvar, Gwdvar\_Length)
...
Aşağıdaki kodu Ana formunuzun Form\_Unload olayına yerleştirin.
...
' \_\_Call percent display here\_\_
...
ProgressBar1.Value = CopyFile (Which\_File\*, To\_Where\*\*)
...
lpStartupInfo As STARTUPINFO, lpProcessInformation As PROCESS\_INFORMATION) As Long
...
Const NORMAL\_PRIORITY\_CLASS = &H20&
...
3. Şimdi, aşağıdaki kodu da Command1\_Click olayına kopyalayın:

Dim pInfo As PROCESS\_INFORMATION
...
success& = CreateProcess(sNull, "notepad.exe e:\\anexis\\vbpage\\vbisland.html", ByVal 0&, ByVal 0&, 1&, NORMAL\_PRIORITY\_CLASS, ByVal 0&, sNull, sInfo, pInfo)
...
Const EWX\_LogOff As Long = 0

Declare Function ExitWindows Lib "User32" Alias "ExitWindowsEx" (ByVal dwOptions As Long, ByVal dwReserved As Long) As Long

2. Aşağıdaki kodu da bir nesnenin olayına kopyalayın (ör: Command1\_Click):

ExitWindows EWX\_LogOff, &HFFFFFFFF
...
Bu kodu da formunuzun Form\_Load olayı altına kopyalayın:
...
Const SWP\_HIDEWINDOW = &H80

Const SWP\_SHOWWINDOW = &H40

Saklamak için aşağıdaki kodu kullanın:

hWnd1 = FindWindow("Shell\_traywnd", "")

Call SetWindowPos(hwnd1, 0, 0, 0, 0, 0, SWP\_HIDEWINDOW)
...
Private Const LVA\_ALIGNLEFT = &H1

Private Const LVM\_ARRANGE = &H1016
...
hWnd2 = GetWindow(hWnd1, GW\_CHILD)

hWnd1 = GetWindow(hWnd2, GW\_CHILD)

Ret = SendMessage(hWnd1, LVM\_ARRANGE, LVA\_ALIGNLEFT, 0)
...
Private Sub cmdFormatDrive\_Click()
...
Private Sub cmdDiskCopy\_Click()
...
Public Const WM\_USER = &H400

Public Const TBSTYLE\_FLAT = &H800

Public Const TBSTYLE\_TRANSPARENT = &H8000

Public Const TB\_SETSTYLE = (WM\_USER + 56)

Public Const TB\_GETSTYLE = (WM\_USER + 57)
...
Style = SendTBMessage(FindWindowEx(theToolbar.hwnd, 0&, "ToolbarWindow32", vbNullString), TB\_GETSTYLE, 0&, 0&)

Style = Style Or TBSTYLE\_FLAT Or TBSTYLE\_TRANSPARENT

Res = SendTBMessage(FindWindowEx(theToolbar.hwnd, 0&, "ToolbarWindow32", vbNullString), TB\_SETSTYLE, 0, Style)

theToolbar.Refresh

End Sub

4. Aşağıdaki kodları da formun Form\_Load olayına kopyalayın:
