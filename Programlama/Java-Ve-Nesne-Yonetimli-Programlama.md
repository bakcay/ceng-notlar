...
public abstract class sekil {
    ...
    public abstract String isimoku();
}
```
...
```java
public class noktaX extends sekil {
...
public class daireX extends noktaX{
    ...
    public daireX(double a, double b, double r) {
        super(a,b);
        yaricap=r;
    }
    public String isimoku(){ return "Daire";}
    public double alan() {return Math.PI*Math.pow(yaricap,2);}
}
```
...
public class silindirX extends daireX{
    ...
    public silindirX(double a, double b, double r, double h) {
        super(a,b,r);
        yukseklik=h;
    }
    public String isimoku(){ return "Silindir";}
    public double alan() {return 2*super.alan() + yukseklik * 2*Math.PI*yaricap;}
...
class soyutdemo {
    public static void main(String args[]) {
        ...
        daireX d;
        silindirX s;
        n=new noktaX(2.0, 1.0);
        d=new daireX(3.0, 4.0, 10.0);
        s=new silindirX(2.0, 4.0, 3.0, 4.0);
        sekiller[0]=n;
        sekiller[1]=d;
        sekiller[2]=s;
        int i;
        for(i=0;i<sekiller.length;i++) {
            System.out.println("Nesne....: " + sekiller[i].isimoku()+ " "+sekiller[i].alan() + " " +sekiller[i].hacim());
        } } }
```
...
```
C:\java\prog\java soyutdemo
Nesne….: Nokta 0.0 0.0
Nesne….: Daire 314.15 0.0
Nesne….: Silindir 131.95 113.09
```
...
```java
class dene implements nehaber {
String isimoku() { // Burada içerik var
}}
```
...
class callpaket {
public static void main(String args[]) {
paket0 p0=new paket0();
}
}
```

```bash
C:\java\prog\paketim\javac paket0.java
C:\java\prog\java paketim.paket00
pakettesin
```

```bash
C:\java\progjava\javac callpaket.java
C:\java\progjava \java callpaket
merhaba
```
